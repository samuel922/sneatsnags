...
export const AdminEventsPage: React.FC = () => {
  ...
  const handleCreateEvent = async (eventData: EventFormData) => {
    try {
      ...
      if (!eventData.name || eventData.name.trim().length < 3 || eventData.name.trim().length > 200) {
      ...
      const backendSections = eventData.sections.map(section => {
        ...
        if (!section.name?.trim() || section.name.trim().length < 1 || section.name.trim().length > 100) {
          throw new Error('Section name must be between 1 and 100 characters long');
        }
        
        // Calculate rowCount and seatCount
        const seatCount = section.capacity || 0;
        const rowCount = Math.max(1, Math.ceil(seatCount / 20)); // Assume 20 seats per row
        
        // Validate priceLevel
        let priceLevel: number | undefined;
        if (section.minPrice && section.minPrice > 0) {
          priceLevel = Math.max(1, Math.min(10, Math.round(section.minPrice / 10))); // Convert price to 1-10 scale
        }
      ...
      // Calculate pricing from sections
      const validPrices = eventData.sections
        .filter(s => s.minPrice && s.minPrice > 0)
        .map(s => ({ min: s.minPrice, max: s.maxPrice || s.minPrice }));
      
      const minPrice = validPrices.length > 0 ? Math.min(...validPrices.map(p => p.min)) : undefined;
      const maxPrice = validPrices.length > 0 ? Math.max(...validPrices.map(p => p.max)) : undefined;
      
      // Calculate total seats
      const totalSeats = eventData.sections.reduce((sum: number, section: SectionFormData) => sum + (section.capacity || 0), 0);
      ...
      const requestData: CreateEventRequest = {
        ...
        imageUrl: eventData.imageUrl && eventData.imageUrl.trim().length > 0 ? eventData.imageUrl.trim() : undefined,
        minPrice,
        maxPrice,
            ...
            <div className="p-6">
              ...
              <div className="space-y-3 mb-4">
                ...
                <div className="flex items-center text-sm text-gray-600">
                  ...
                  <span className="font-medium">{event.totalSeats || 0} capacity</span>
                </div>
                {(event.minPrice || event.maxPrice) && (
                  <div className="flex items-center text-sm text-gray-600">
                    ...
                    <span className="font-medium">{formatCurrency(event.minPrice || 0)} - {formatCurrency(event.maxPrice || 0)}</span>
                  </div>
                )}
              </div>
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <div className="space-y-2">
                        {selectedEvent.sections?.map((section: EventSection) => (
                          <div key={section.id} className="flex justify-between items-center p-3 bg-gray-50 rounded">
                            <div>
                              <div className="font-medium">{section.name}</div>
                              <div className="text-sm text-gray-600">{section.capacity} seats</div>
                            </div>
                            <div className="text-right">
                              <div className="font-medium">Capacity: {section.capacity}</div>
                              <div className="text-sm text-gray-600">Active: {section.isActive ? 'Yes' : 'No'}</div>
                            </div>
                          </div>
                        ))}
                      </div>
